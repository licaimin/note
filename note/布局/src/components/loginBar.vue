<template>
	<div class="login_bar">
    			<div class="tab_bar">
    				...
    				<img src="../../static/imgs/cancel.png" class="cancel_icon" @click="cancel"/>
    			</div>
    			<div class="form_bar">
    				<!-- 登陆表单 -->
    				<el-form :model="ruleForm"  :rules="rules" ref="ruleForm"  class="form" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="加载中..." v-if='status' key='form1'>
					  ...
					  <el-form-item prop="password">
					    <el-input :type="passVisible1?'text':'password'" v-model="ruleForm.password" autocomplete="on" placeholder='请输入密码'  class='loginIpt' ></el-input>
    				...
    				<el-form :model="ruleForm2"  :rules="rules2" ref="ruleForm2"  class="form" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="加载中..." v-if='!status' key='form2'>
					  ...
					  <el-form-item prop="checkpass">
					    <el-input :type="passVisible2?'text':'password'" v-model="ruleForm2.checkpass" autocomplete="off" placeholder='请再次输入密码'  class='loginIpt'></el-input>
		...
		methods:{
			hoverFn(idx){
      		...
      		selfReq(){
      			...
      			if(token){
      				var userId=(JSON.parse(atob(token.split('.')[1]))).jti
			...
			submitForm(form) {
      		...
      		loginReq(res){
        		...
        		this.$axios({
        			...
        			if(res.data.code==20000){
				        ...
				        let userData=JSON.stringify(res.data.data.user)
      		...
      		cancel(){
				...
				this.$emit('listenCancel',this.isLgShow)
			},
			tab1(){
				this.tab_='tab1'
				this.status=true
