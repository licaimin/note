          ...
          <div class="main ">
              <!--作品详情和评论-->
                ...
                <div class="article-content common-style">
                    <p>{{articleContent1}}</p>
                    <li><img :src="articleImgs[0].article1"></li>  
                    <p>{{articleContent2}}</p>
                    <li><img :src="articleImgs[1].article2"></li>  
                </div> <!--文章的主要内容-->
            <div class="comment common-style ">
                ...
                <p class="font-em" style="text-align:center; margin-top:32px">没有更多评论了...</p>
            </div> <!--评论区域-->
          </div>
          <div class="side ">
              <ul class="user  common-style">
                  ...
                  <li> <button type="button" class="btn btn-primary user-btn btn-my-style" v-show="!showLike" @click="likeFun('like')">关注</button></li>
                  <li> <button type="button" class="btn btn-primary user-btn btn-my-style" v-show="showLike" @click="likeFun('cancelLike')">已关注</button></li>
              </ul>
...
//   });
  export default {
    ...
    created(){
        this.commentUser.fromId = '1097402180014456832';
        this.commentUser.fromAvatar = this.imgs.head;
        this.commentUser.fromName = '测试'
    ...
    //    this.$router.push({path: '/messagememberlist',query:{
    //     recruitViperId: this.listSecretary[index].recruitViperId ,
    //    }},
    //    );
        //获取文章页面
        getDetail(){
            this.$axios.get(this.domainName + '/work/'+this.ownerId,{
                // params:{
                ...
                // }
                 ...
                 if(res.data.code === 20000){
                    ...
                    //this.updateTime = res.data.data.work.updateTime;//更新时间????????
                     this.visits = res.data.data.work.visits;//浏览数
                     this.likeNum = res.data.data.work.likeNum;//点赞数
                     this.favouriteNum = res.data.data.work.favouriteNum;//收藏数
                     this.commentNum = res.data.data.work.commentNum;//评论数

                    //作者详情
                    //  id = (res.data.relationVO.id;//作者ID
                     this.imgs.head = res.data.data.relationVO.avatar;//作者头像
                     this.nickname = res.data.data.relationVO.nickname;//作者的用户名
                     this.personalDescription = res.data.data.relationVO.personalDescription;//作者的简介
                     this.islike= res.data.data.relationVO.islike;//是否关注了作者。-1表示没有关系，0表示关注 1表示互相关注
                    //  this.imgs.head = res.data.relationVO.followTime;//???????
                    if(this.islike === -1){
                        //未关注
                        this.showLike =  false;
        ...
        getComment(value){
             this.$axios({
                method:'get',
                url:this.domainName+ '/comment/'+this.ownerId,
                params:{
                    sortType:value
                },
                headers: {
            ...
            //  this.$axios.get(this.domainName + ' /comment/{'+this.$route.query.ownerId+'}')
        },
               ...
               this.$message({
                ...
                let postData={
                    ownerId:this.ownerId,
                        //发布评论者
                        fromId:this.commentUser.fromId,//ID
                        fromName:this.commentUser.fromName,//昵称
                        fromAvatar:this.commentUser.fromAvatar,//头像 
                        //评论对象
               ...
               this.$message({
                ...
                let postData = {
                    ownerId:this.ownerId,
                        ...
                        fromAvatar:this.commentUser.fromAvatar,//头像 
                        // 评论对象
                        told:item.fromId,//null 表示评论作品，不是回复
                ...
                item.answerUsername = this.commentUser.fromName; //这里插入的是文章的作者名
                // this.commentIndex = -1;
            ...
            for(let i= 0; i< length; i++){
                ...
                this.$axios({
                    ...
                    this.$set(this.comment[i],'showCall',res.data.data)                     
                        ...
                        // console.log(this.comment[i].showCall);
                })
        }
        ...
        getCallWork(){
             this.$axios({
                method:'get',
                url:this.domainName+'/work/islike/'+this.ownerId,
                headers:{
                        'Content-Type': 'application/x-www-form-urlencoded',
                }
            }).then((res)=>{
                console.log(res)
                if(res.data.data){
                    this.showCallWork = true;
                }else{
                    this.showCallWork = false;
                }
            }).catch((error)=>{
