<template>
	<div class="outcome">
		<div class="box">
			<div class='itemCtn'>
				<card v-for='item of $store.state.outcomeList' :key='item.id' v-bind:item='item' @click='cardFn(item.id)'></card>
			</div>
			<div class="pageCtn">
		...
		methods:{
        	...
        	//获得搜索结果
      		searchReq(page){
        		...
        		this.$axios({
        			method:'get',
        			url:'/api/work/search?key='+key+'&pageNum='+page+'&pageSize=20&sort='+sort+'&type='+type,
        		}).then((res)=>{
        			if(res.code==200){
        				this.$store.dispatch('total',res.data.total)
				        this.$store.dispatch('search',res.data.rows)
        			}else{
